...
def main(args: Namespace):
    ...
    for train_step in tqdm(range(args.num_train_steps)):
        ...
        with torch.no_grad():
            ...
            v_base_all = base_model(x_in, t_in, y_in)
            v_base_cond, v_base_uncond = v_base_all.chunk(2)
            v_target = v_base_uncond + args.cfg_scale * (v_base_cond - v_base_uncond)

        v_draft = draft_model(x_in, t_in, y_in)
        v_draft_cond, v_draft_uncond = v_draft.chunk(2)
        v_pred = v_draft_uncond + cfg_scale * (v_draft_cond - v_draft_uncond)
