...
def model_call_cfg_from_teacher_hidden(base_model, draft_model, x, t, y, y_null, cfg_scale: float):
    batch_size = x.shape[-4]
    channels, height, width = x.shape[-3:]
    leading_shape = x.shape[:-4]
    ...
    _, (hidden_tokens_flat, hidden_cond_flat) = base_model(
        x_model_flat,
        t_model_flat,
        y_model_flat,
        return_hidden=True,
    ...
    draft_first_param = next(draft_model.parameters(), None)
    if draft_first_param is not None:
        hidden_tokens_flat = hidden_tokens_flat.to(device=draft_first_param.device, dtype=draft_first_param.dtype)
        hidden_cond_flat = hidden_cond_flat.to(device=draft_first_param.device, dtype=draft_first_param.dtype)

    v_flat = draft_model.forward_from_teacher_hidden(hidden_tokens_flat, hidden_cond_flat)
    v_flat = v_flat.to(device=x.device, dtype=x.dtype)
    v = v_flat.reshape(*leading_shape, 2, batch_size, channels, height, width)
    v_cond = v.select(dim=-5, index=0)
    v_uncond = v.select(dim=-5, index=1)
...
def speculative_trajectory_projected(
    base_model,
    draft_model,
    x,
    y,
    ...
    threshold: float,
    show_progress: bool = False,
    progress_desc: str = "speculative-projected",
):
    """
    Speculative trajectory where draft predictions come from projected base hidden states.
    """
    device = x.device
    batch_size = x.shape[0]
    ...
    for i in iter_range:
        ...
        for j in range(1, num_draft_steps + 1):
            v_draft_final = model_call_cfg_from_teacher_hidden(
                base_model,
                draft_model,
                next_draft_traj[j - 1],
                t_model,
