...
def speculative_trajectory_proj_draft(
    base_model,
    draft_model,
    x,
    y,
    ...
    threshold: float,
    show_progress: bool = False,
    progress_desc: str = "speculative-proj-draft",
):
    """
    Copy of speculative_trajectory, except draft velocity generation is:
    base forward (to get hidden) -> projected draft forward.
    """
    assert num_draft_steps == 1, (
        "Projected draft mode currently supports one draft iteration: "
        "run base once to get hidden, then one draft speculation."
    )
    ...
    for i in iter_range:
        ...
        next_draft_traj[1] = x_draft_traj
...
# Backwards-compatible alias for the previous typo.
def speceulative_trajectory_proj_draft(*args, **kwargs):
    return speculative_trajectory_proj_draft(*args, **kwargs)


def small_then_large_trajectory(
    draft_model,
    base_model,
    x,
    y,
    ...
    small_iters: int = 10,
    show_progress: bool = False,
    progress_desc: str = "small-then-large",
):
    """
    Run the small model for a fixed number of Picard iterations, then switch to the base model.
    """
    batch_size = x.shape[0]
    dt = 1.0 / num_steps
    ...
    x_traj = x_traj_0.clone()
    residual = torch.tensor(float("inf"), device=x.device)
    residual_history_small = []
    residual_history_big = []

    small_iters_run = 0
    for _ in range(min(small_iters, num_steps)):
        v_final = model_call_cfg(draft_model, x_traj, t_model, y_traj, y_null_traj, cfg_scale)
        x_traj_new = x_traj_0.clone()
        x_traj_new[1:] = x_traj_new[1:] + torch.cumsum(v_final[:-1], dim=0) * dt
        residual = torch.max(torch.mean(torch.abs(x_traj_new - x_traj), dim=(2, 3, 4)))
        residual_history_small.append(float(residual.item()))
        x_traj = x_traj_new
        small_iters_run += 1
    ...
    for _ in iter_range:
        ...
        x_traj_new[1:] = x_traj_new[1:] + torch.cumsum(v_final[:-1], dim=0) * dt
        residual = torch.max(torch.mean(torch.abs(x_traj_new - x_traj), dim=(2, 3, 4)))
        residual_history_big.append(float(residual.item()))
        x_traj = x_traj_new
        big_iters_run += 1
        if residual < threshold:
            break
    ...
    return x_traj[-1], {
        "iters": small_iters_run + big_iters_run,
        "small_iters": small_iters_run,
        "big_iters": big_iters_run,
        "residual": float(residual.item()),
        "small_residual_history": residual_history_small,
        "big_residual_history": residual_history_big,
    }
